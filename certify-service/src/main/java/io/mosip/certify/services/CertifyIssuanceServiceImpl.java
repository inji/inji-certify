...
public class CertifyIssuanceServiceImpl implements VCIssuanceService {
    ...
    public CredentialResponse getCredential(CredentialRequest credentialRequest) {
        ...
        for (String holderId : holderIds) {
            vcResults.add(getVerifiableCredential(credentialConfigurationSupported, holderId, credentialRequest));
        }
        ...
        try {
            ...
            switch (format) {
                ...
                CredentialDefinition credentialDefinition =
                    credentialRequest.getCredentialDefinition();

                if (credentialDefinition == null) {
                    throw new CertifyException(
                            VCIErrorConstants.INVALID_CREDENTIAL_REQUEST,
                            "credential_definition required for jwt_vc_json");
                }

vcRequestDto.setContext(credentialDefinition.getContext());
vcRequestDto.setType(credentialDefinition.getType());
vcRequestDto.setCredentialSubject(
        credentialDefinition.getCredentialSubject());
                    templateName = CredentialUtils.getTemplateName(vcRequestDto);
