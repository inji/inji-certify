...
public class VCIssuanceServiceImpl implements VCIssuanceService {
    ...
    public CredentialResponse getCredential(CredentialRequest credentialRequest) {
        ...
        for(String scope : scopeClaim.split(Constants.SPACE)) {
            Optional<CredentialMetadata> result = VCIssuanceUtil.getScopeCredentialMapping(scope, credentialRequest.getFormat(), credentialConfigurationService.fetchCredentialIssuerMetadataV2(), credentialRequest);
            if(result.isPresent()) {
